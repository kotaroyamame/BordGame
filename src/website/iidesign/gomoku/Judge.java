		...
		if(count[1][1]==1){
			out: for (int i = 1; i <= 8; i++) {
				...
				for (int j = 1; j < 4 + k; j++) {
					...
					if (mindBord.sarchMind(x, y, j, i) == storn && j == (3 + k) - count[0][1] && 
							((count[0][1] != 0&&(mindBord.sarchMind(x, y, j + 1+k, i) == -1 || mindBord.sarchMind(x, y, -j - 1, i) == -1))
									|| (count[0][1] == 0&&(mindBord.sarchMind(x, y, 0, i) == -1 || mindBord.sarchMind(x, y, j + 1+k, i)== -1)))) {
						System.out.println("し x="+x+" y="+y);
						if (count[0][1] == 0) {
						...
						count[0][0]++;
					} else if (mindBord.sarchMind(x, y, j, i) == -1 && count[1][0] < 1) {// 飛び石四
						count[1][0]++;
						k = 1;
					} else {
						count[0][1] = i % 2 == 0 ? 0 : count[0][0];
